...
void product(double* c, const double* a, const double* b, long N){
  double s = 0;
...
__global__ void product_gpu(double* c, const double* a, const double* b, long N){
  ...
  if (threadIdx.x <  32) {
    ...
    __syncwarp();
    if (threadIdx.x == 0) c[blockIdx.x] = smem[0] + smem[1];
  }
}
...
int main() {
  ...
  product_gpu<<<Nb,BLOCK_SIZE>>>(sum_d, x_d, y_d, N);
  while (Nb > 1) {
    long N = Nb;
  ...
  cudaMemcpyAsync(&c, sum_d, 1*sizeof(double), cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
